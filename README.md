...
项目国际化翻译解决方案，支持Android、iOS、Flutter、前端Vue、后端PHP等等，点几下按钮就能实现翻译内容的自动抓取和翻译后文件的自动生成，适合各类场景下的国际化需求。目前项目正在快速适配和推广阶段，欢迎广大程序员加入。项目主页：https://github.com/MZCretin/Eva-Translate
官网入口：https://cretinzp.com/pro-translate
...
一个提供开发中常用数据的一个稳定聚合Api接口源，运行于独立服务器，此服务从2018年11月19日第一个用户开始成功调用接口，到现在已经稳定运行超过7个年头，服务器也升级换代了十几次，提交代码数千次。接口会秉承免费，且长期维护，和持续添加新的接口！
...
<div style="background:#e3e3e3; color:#FFF" align=center ><img width="200" height="300" src="./png/me.jpeg"/><img width="150" height="300" src="./png/gongzhonghao1.jpg"/></div>
...
文档同步更新，补充 2024 年以来陆续新增的接口（本仓库仅作文档索引，各接口的请求参数与返回示例请以文档主页为准：https://www.mxnzp.com/doc/list ）：

+ 新增抖音数据专业版系列接口（视频详情/去水印、视频与用户搜索、评论及子评论、用户作品与收藏、粉丝画像与数据分析、热榜榜单等）
+ 新增小红书内容解析接口（视频/图文详情、用户信息与作品、笔记与用户搜索、评论列表等）
+ 新增快手视频解析接口（去水印，多版本）
+ 新增 AI 相关接口：AI 对话、AI 文生图 / 图生图、AI 识图（图片解释）
+ 新增豆包 / 千问 图文生成解析接口
+ 新增随机视频接口
+ 新增数学表达式计算接口
+ 新增短信验证码接口（自定义 / 自动模板发送、校验）
+ 新增微信 / 微信群消息通知接口
+ 新增直播活码（动态二维码）接口及配套控制台数据接口
+ 新增食物热量 V2 接口

变更与下线说明：

+ 「音乐相关接口」「物流查询接口」因上游数据源不再稳定，已停止维护并下线，文档保留仅作历史记录，不再可用
+ 「全国城市列表」已升级为多级地区接口（省/市/区/街道/村，v3），原 city 相关能力由地区接口与「世界电话区号列表」承接
+ 「在线自定义参数」已重构为远程配置（remote_config）接口
+ 天气、新闻、条形码、二维码、福利图片、驾考、汇率等多个已有接口均新增了 v2/v3 版本，详见文档主页
